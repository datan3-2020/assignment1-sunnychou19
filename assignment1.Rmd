...
library(tidyverse)
getwd()
Data <- read_tsv('C:/Users/user/Desktop/data/UKDA-6614-tab/tab/ukhls_w8/h_indresp.tab')
```
...
```{r}
Data <- Data %>% select(pidp, h_eumem, h_sex_dv, h_age_dv, h_memorig)
...
Data <- Data %>%
        ...
        mutate(EU = recode(h_eumem, '1' =  '1',
                           '2'=  '0',
                           .default = NA_character_)) %>%
  
        mutate(sex =  recode (h_sex_dv, '1' = 'Male', '2' = 'Female',.default = NA_character_)) %>%
  
        mutate(agegr = case_when(between(h_age_dv, 16,25) ~ '16 to 25',
                                 between(h_age_dv, 26,40) ~ '26 ro 40',
...
Data %>%   count(EU) %>% mutate(perc = n / sum(n) * 100)
...
The results of the 2016 referendum are 52% for leave and 48 % for remain, with leave scoring 4% more than remain ; the results here are 40.7% for leave and 48.4% for remain, with remain scoring 7.7% greater than leave.  Possible reasons are that the results are within the confidence interval of the population, where results could reasonably vary between a certain range within a number of samples. The difference between the two could also be accounted by the time passed, thus post-referendum information received and consequences contemplated. 
...
Data %>% select(EU,sex, agegr) %>% group_by(sex,agegr) %>% summarise(propEU = mean(EU, na.rm = TRUE) * 100)
    

tapply(Data, c(Data$EU,sex,agegr),mean )
        ...
        
```

Youngness correlated to likeliness to support remain, female more likely than male in almost each age group. 
